...
func TransferWorkflowV4(ctx workflow.Context, state messages.Transfer) (err error) {
	...
	for {
		...
		selector.AddReceive(verifyOtpChannel, func(c workflow.ReceiveChannel, _ bool) {
			...
			// TODO: Test only
			err = workflow.ExecuteActivity(ctx, a.NewActivityForTest22, state).Get(ctx, nil)
			if err != nil {
				logger.Error("Error execute activity NewActivityForTest22: %v", err)
				return
			}
			// Compensation
			// defer func() {
